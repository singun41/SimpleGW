...
async function update() {
    ...
    if(docsId) {
        saveComplete = true;
        location.href = `/page/approval/forms/${docsType}/${docsId}`;   // common/modify.js와 url이 다르다.
    }
}
...
async function updateApprovalDocs(params) {
    ...
    let response = await fetchPatchParams(`approval/forms/${docsType}/${docsId}`, params);   // common/modify.js와 url이 다르다.
    let result = await response.json();
